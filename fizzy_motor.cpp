...
int FizzyMotor::pinValue(MotorInfo* motor, uint8_t brake_force) {

    int pin_val = reversed_speed + motor->calib_value + brake_force;

    return pin_val > FIZZY_MOTOR_PIN_HIGH ? FIZZY_MOTOR_PIN_HIGH : pin_val;
}

void FizzyMotor::forwardWheel(MotorInfo* motor, uint8_t brake_force = 0) {


    analogWrite(motor->pins[0], pinValue(motor, brake_force));
    analogWrite(motor->pins[1], FIZZY_MOTOR_PIN_HIGH);
}

void FizzyMotor::backwardWheel(MotorInfo* motor, uint8_t brake_force = 0) {

    analogWrite(motor->pins[0], FIZZY_MOTOR_PIN_HIGH);
    analogWrite(motor->pins[1], pinValue(motor, brake_force));
}
...
void FizzyMotor::brakeWheel(uint8_t force, Motor m) {
...
void FizzyMotor::forwardWheel(Motor m, uint8_t brake_force) {

    switch(m) {
    
    case LeftMotor:
        forwardWheel(&motorLeft, brake_force);
        fizzy_state = FIZZY_WHEEL_STATE(FIZZY_MASK_MOTOR_LEFT,
                                        motorState(FIZZY_STATE_MOTOR_FORWARD, LeftMotor));
        break;

    case RightMotor:
        forwardWheel(&motorRight, brake_force);
...
void FizzyMotor::backwardWheel(Motor m, uint8_t brake_force){

    switch(m) {
    
    case LeftMotor:
        backwardWheel(&motorLeft, brake_force);
        fizzy_state = FIZZY_WHEEL_STATE(FIZZY_MASK_MOTOR_LEFT,
                                        motorState(FIZZY_STATE_MOTOR_BACKWARD, LeftMotor));
        break;

    case RightMotor:
        backwardWheel(&motorRight, brake_force);
        fizzy_state = FIZZY_WHEEL_STATE(FIZZY_MASK_MOTOR_RIGHT,
...
void FizzyMotor::brakeLeft(int force_max255) {
    brakeWheel(force_max255, LeftMotor);
}

void FizzyMotor::brakeRight(int force_max255) {
    brakeWheel(force_max255, RightMotor);
}
